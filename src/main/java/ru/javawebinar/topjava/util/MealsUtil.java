...
public class MealsUtil {
    public static final List<Meal> MEALS = Arrays.asList(
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 30, 10, 0), "Завтрак", 500),
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 30, 13, 0), "Обед", 1000),
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 30, 20, 0), "Ужин", 500),
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 31, 10, 0), "Завтрак", 1000),
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 31, 13, 0), "Обед", 500),
            new Meal(null, AuthorizedUser.id(), LocalDateTime.of(2015, Month.MAY, 31, 20, 0), "Ужин", 510)
    );
    ...
    public static List<MealWithExceed> getFilteredWithExceeded(List<Meal> meals, LocalDate startDate, LocalDate endDate, int caloriesPerDay) {
        Map<LocalDate, Integer> sumOfCaloriesForDay = meals
                .stream()
                .collect(Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum));
        return meals
                .stream()
                .filter(meal -> DateTimeUtil.isBetween(meal.getDate(), startDate, endDate))
                .map(meal -> createWithExceed(meal, sumOfCaloriesForDay.get(meal.getDate()) > caloriesPerDay))
